...
from trajdata.maps.vec_map_elements import (
    PedCrosswalk, Polyline, RoadLane, RoadArea)
from trajdata.data_structures.batch_element import (
    AgentBatchElement, SceneBatchElement)
from trajdata.data_structures.agent import (
    AgentMetadata, AgentType, VariableExtent)
...
def fetch_season_info(
        batch_element: Union[AgentBatchElement, SceneBatchElement]) -> int:
...
def get_scene_path(data_dir: str, scene_idx: int) -> str:
    return os.path.join(
        data_dir,
        str(scene_idx // 1000).zfill(3),
        str(scene_idx).zfill(6)) + ".pb"
...
def fix_headings(agents_data_df: pd.DataFrame) -> pd.DataFrame:
    ...
    normalized_angle_diff = \
        np.abs(headings - previous_headings) % (2 * np.pi)
...
def fill_missing_timestamps(
        ...
        agent_id_to_time_range: dict) -> pd.DataFrame:
    filled_agents_data_df = []
    for agent_id, agent_df in agents_data_df.groupby('agent_id'):
        state_idx = 0
        for ts in range(
                agent_id_to_time_range[agent_id][0],
                agent_id_to_time_range[agent_id][1] + 1):
            if state_idx < agent_df.shape[0] and \
                    agent_df.iloc[state_idx]["scene_ts"] == ts:
                ...
                filled_agents_data_df.append({
                    ...
                    "width": None,
                    "height": None,})
    return pd.DataFrame(filled_agents_data_df).sort_values(
        by=["agent_id", "scene_ts"])


def map_ysdc_to_trajdata_traffic_light_status(
        ysdc_tl_status: int) -> TrafficLightStatus:
    mapping = {
...
def extract_traffic_light_status(ysdc_scene: YSDCScene) -> \
        Dict[Tuple[str, int], TrafficLightStatus]:
    traffic_light_data = {}
    n_states = len(ysdc_scene.past_vehicle_tracks) + \
        len(ysdc_scene.future_vehicle_tracks)
    ...
    for traffic_light in ysdc_scene.traffic_lights:
        for traffic_light_section in traffic_light.sections:
            traffic_light_section_id_to_state[traffic_light_section.id] = \
    ...
    for lane_idx, lane in enumerate(ysdc_scene.path_graph.lanes):
        ...
        ysdc_traffic_light_state = \
            traffic_light_section_id_to_state[lane_main_section_id]
        conventional_traffic_light_state = \
            map_ysdc_to_trajdata_traffic_light_status(ysdc_traffic_light_state)
        for ts in range(n_states):
            traffic_light_data[(conventional_lane_id, ts)] = \
                conventional_traffic_light_state
    return traffic_light_data


def extract_vectorized(
        map_features: YSDCPathGraph, map_name: str) -> VectorMap:
    ...
    for crosswalk_idx, crosswalk in enumerate(map_features.crosswalks):
        ...
        vec_map.add_map_element(
            PedCrosswalk(
                ...
                polygon=Polyline(crosswalk_points)))

    for road_polygon_idx, road_polygon in enumerate(
            map_features.road_polygons):
...
def prepare_agent_info_dict_from_track(
        track: YSDCVehicleTrack, scene_ts: int,
        entity: AgentType, is_ego: bool = False) -> Dict[str, Any]:
    ...
    return {
        ...
        "ax": 0 if entity==AgentType.PEDESTRIAN else \
            track.linear_acceleration.x,
        "ay": 0 if entity==AgentType.PEDESTRIAN else \
            track.linear_acceleration.y,
        "heading":
            np.arctan2(track.linear_velocity.y, track.linear_velocity.x) \
                if entity==AgentType.PEDESTRIAN else track.yaw,
...
def update_time_range(
        agent_id: str, timestamp: int,
        agent_id_to_time_range: Dict[str, Tuple[float, float]]) -> None:
...
def extract_agent_data_from_ysdc_scene(
        ysdc_scene: YSDCScene, trajdata_scene: TRAJScene) -> \
            Tuple[
                pd.DataFrame, List[AgentMetadata], List[List[AgentMetadata]]]:
    ...
    ego_agent_data = list(ysdc_scene.past_ego_track) + \
        list(ysdc_scene.future_ego_track)
    for agent_type, scene_moment_states in agents_types_data:
        for timestamp, scene_moment_state in enumerate(
                scene_moment_states):
            for agent_moment_state in scene_moment_state.tracks:
                ...
                scene_agents_data[agent_info_dict["agent_id"]] \
                    .append(agent_info_dict)
                update_time_range(
                    agent_info_dict["agent_id"], timestamp,
                    agent_id_to_time_range)
                agent_id_to_type[str(agent_moment_state.track_id)] = \
                    agent_type
    ...
    scene_agents_data_df = pd.DataFrame([
        item for sublist in scene_agents_data.values() for \
                item in sublist]) \
    ...
    scene_agents_data_df = scene_agents_data_df.groupby(
            "agent_id", group_keys=True) \
        .apply(lambda group: group.interpolate(limit_area="inside")) \
        .reset_index(drop=True)
    for agent_id in agent_id_to_type.keys():
        agent_list.append(AgentMetadata(
            name=agent_id,
        ...
        for ts in range(
                agent_id_to_time_range[agent_id][0],
                agent_id_to_time_range[agent_id][1] + 1):
            agent_presence[ts].append(AgentMetadata(
                name=agent_id,
                agent_type=agent_id_to_type[agent_id],
                first_timestep=agent_id_to_time_range[agent_id][0],
                last_timestep=agent_id_to_time_range[agent_id][1],
