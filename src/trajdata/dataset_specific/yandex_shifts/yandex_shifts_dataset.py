...
from trajdata.data_structures import (
    EnvMetadata, Scene, SceneMetadata, SceneTag)
...
from trajdata.dataset_specific.yandex_shifts.yandex_shifts_utils import (
    read_scene_from_original_proto, get_scene_path, extract_vectorized,
    extract_traffic_light_status, extract_agent_data_from_ysdc_scene)
...
class YandexShiftsDataset(RawDataset):
    def compute_metadata(self, env_name: str, data_dir: str) -> EnvMetadata:
        if env_name == "ysdc_train":
            ...
            scene_split_map = defaultdict(
                partial(const_lambda, const_val="train"))

        elif env_name == "ysdc_development":
            dataset_parts = [("development",)]
            scene_split_map = defaultdict(partial(
                const_lambda, const_val="development"))

        elif env_name == "ysdc_eval":
            dataset_parts = [("eval",)]
            scene_split_map = defaultdict(partial(
                const_lambda, const_val="eval"))

        elif env_name == "ysdc_full":
            dataset_parts = [("full",)]
            scene_split_map = defaultdict(partial(
                const_lambda, const_val="full"))
    ...
    def load_dataset_obj(self, verbose: bool = False) -> None:
        ...
        self.dataset_obj = scenes_generator(
            get_file_paths(self.metadata.data_dir))
    ...
    def _get_matching_scenes_from_obj(
        ...
        for idx, scene in tqdm.tqdm(
                enumerate(self.dataset_obj),
                desc="Processing scenes from proto files"):
    ...
    def _get_matching_scenes_from_cache(
        ...
        all_scenes_list: List[YandexShiftsSceneRecord] = \
            env_cache.load_env_scenes_list(self.name)
        scenes_list: List[SceneMetadata] = list()
    ...
    def get_scene(self, scene_info: SceneMetadata) -> Scene:
        ...
        num_future_timestamps = \
            len(scene_data_from_proto.future_vehicle_tracks)
        ...
        return Scene(
            ...
            data_idx,
            {
                ...
                "precipitation":
                    scene_data_from_proto.scene_tags.precipitation})
    ...
    def get_agent_info(
        ...
        scene_agents_data_df, agent_list, agent_presence = \
    ...
    def cache_map(
        ...
        vector_map: VectorMap = extract_vectorized(
            scene_data_from_proto.path_graph,
